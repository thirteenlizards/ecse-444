...
// Sine Wave Array
static volatile uint8_t sine_index = 0;		// index into LUT
static volatile uint8_t sine_wave[SAMPLES];	// LUT
...
int main(void)
{
  ...
  while (sine_index < (SAMPLES)) {
  		    ...
  		    sine_wave[sine_index] = (uint8_t)(value + 0.5f); // round to nearest integer
  		    sine_index++;
  	} // while (sine_index < samples)
